...
Result::Result (QWidget *parent) : QDialog (parent), ui (new Ui::Result)
{
  ui->setupUi (this);
...
void
Result::on_menu_result_button_clicked ()
{
  this->hide ();
...
void
Result::evaluateForLeaderboard (int score)
{
  ui->score_label->setText (QString::number (score));
  loadLeaderboardMembers ();
  auto minElement = std::min_element (m_leaderboardMembers.begin (),
                                      m_leaderboardMembers.end ());
...
void
Result::loadLeaderboardMembers ()
{
  QString appDirPath = QCoreApplication::applicationDirPath ();

  std::ifstream file (appDirPath.toStdString ()
  ...
  if (file.is_open ())
          ...
          if (std::regex_search (line, matches, pattern))
            {
              ...
              m_leaderboardMembers.push_back (
                  std::make_pair (numericValue, stringValue));
            }
        }
      file.close ();
    }
    ...
    {
      std::cerr << "Unable to open file in loadLeaderboardMembers()"
                << std::endl;
    }
}

void
Result::on_enter_result_button_clicked ()
{
  ui->label_2->hide ();
  ui->enter_result_button->hide ();
  ui->result_lineEdit->hide ();
  ...
  if (playerName.isEmpty ())
    playerName = "player";
  m_leaderboardMembers.erase (std::min_element (m_leaderboardMembers.begin (),
                                                m_leaderboardMembers.end ()));
  m_leaderboardMembers.push_back (std::make_pair (
      (ui->score_label->text ()).toInt (), playerName.toStdString ()));
  auto comparePairs
      = [] (const auto &a, const auto &b) { return a.first > b.first; };

  std::sort (m_leaderboardMembers.begin (), m_leaderboardMembers.end (),
             comparePairs);

  QString appDirPath = QCoreApplication::applicationDirPath ();

  std::ofstream outputFile (appDirPath.toStdString ()
                            + "/../MATFWars/resources/txt/leaderboard.txt");
  ...
  if (outputFile.is_open ())
      ...
      for (const auto &pair : m_leaderboardMembers)
        {
          ...
          if (pair.first < 10)
            outputFile << "0";
          outputFile << pair.first << " &nbsp; " << pair.second << " </li>"
                     << std::endl;
        }
      outputFile << "<ol>" << std::endl;
    ...
    {
      std::cerr
          << "Error opening the file in on_enter_result_button_clicked() "
          << std::endl;
    }
}

bool
Result::eventFilter (QObject *obj, QEvent *event)
{
  if (obj == ui->enter_result_button && event->type () == QEvent::Enter)
    {
      ui->enter_result_button->setCursor (Qt::PointingHandCursor);
      ui->enter_result_button->setStyleSheet (
  ...
  else if (obj == ui->menu_result_button && event->type () == QEvent::Enter)
    {
      ...
      ui->menu_result_button->setStyleSheet (
          menuStyle + "border:7px solid rgb(180, 72, 72);");
    }
