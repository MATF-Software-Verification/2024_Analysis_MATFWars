...
WarGame::WarGame (Client *client, QWidget *parent)
    : QDialog (parent), ui (new Ui::WarGame), m_canvas (new Canvas (this)),
      m_client (client)
{
  ui->setupUi (this);
  ptrCheck = new Check ();
  ...
  connect (m_client, &Client::someMessage, this,
           &WarGame::clientReceivedMessage);
  connect (ui->chat_send_button, &QPushButton::clicked, this,
           &WarGame::sendMessage);
  connect (ui->chat_lineEdit, &QLineEdit::returnPressed, this,
           &WarGame::sendMessage);
  ...
  connect (ptrCheck, &Check::noButtonClicked, this, &WarGame::show);
  connect (this, &WarGame::newPlayerIsSet, dynamic_cast<Canvas *> (m_canvas),
           &Canvas::addPlayer);
  connect (this, &WarGame::newObstacleIsSet, dynamic_cast<Canvas *> (m_canvas),
           &Canvas::addObstacle);
  connect (this, &WarGame::setCoordinateSystem,
           dynamic_cast<Canvas *> (m_canvas), &Canvas::addCoordinateSystem);
  connect (this, &WarGame::cleanUpCanvas, dynamic_cast<Canvas *> (m_canvas),
           &Canvas::cleanUp);

  connect (ui->fire_war_button, &QPushButton::clicked, this,
           &WarGame::inputTaken);
  connect (ui->leFunctionInput, &QLineEdit::returnPressed, this,
           &WarGame::inputTaken);
  connect (this, &WarGame::newFunctionIsSet, dynamic_cast<Canvas *> (m_canvas),
           &Canvas::setFunction);
...
void
WarGame::createObstacleJsonArray (QJsonArray *obstaclesJson)
{
...
void
WarGame::fireFunction (std::string fString)
{
  ...
  function->scaleToCanvas (m_canvas->width (), m_canvas->height (),
                           dynamic_cast<Canvas *> (m_canvas)->gridWidth ());
  function->translatePointsObserverView (m_canvas->width () / 2,
                                         m_canvas->height () / 2);

  QRectF sceneRect = QRectF (m_canvas->sceneRect ());
  FunctionNode *node
      = new FunctionNode (function, sceneRect.width (), sceneRect.height ());
  ...
  QTimer::singleShot (2000, [this, function] () {
    if (playerWinner != nullptr)
      ...
      {
        switchPlayer ();
        if ((m_fromCreate && currentPlayer == 0)
            || (!m_fromCreate && currentPlayer == 1))
          {
...
QPointF
WarGame::getFirePosition ()
{
  if (currentPlayer == 0)
    return QPointF (player0->coordinate ().x () + player0->diameter () / 2,
                    player0->coordinate ().y ());
  else
    return QPointF (player1->coordinate ().x () + player1->diameter () / 2,
                    player1->coordinate ().y ());
}

void
WarGame::collisionDetection (Function *function)
{
  int cutoff = 0;

  for (QPointF p : function->points ())
    {
      if (isPointInCircle (p, player0->coordinate (),
                           player0->diameter () / 2))
        {
      ...
      if (isPointInCircle (p, player1->coordinate (),
                           player1->diameter () / 2))
        {
          function->removePointsAfterCutoff (cutoff);
      ...
      for (Obstacle *obstacle : obstacles)
        {
          if (isPointInCircle (p, obstacle->center (),
                               obstacle->diameter () / 2))
            {
...
bool
WarGame::isPointInCircle (QPointF p, QPointF center, double radius)
{
  double distance = std::sqrt (std::pow (p.x () - center.x (), 2)
                               + std::pow (p.y () - center.y (), 2));
  return distance < radius;
}

void
WarGame::generateObstacles (int width, int height)
{
...
bool
WarGame::allowedObstacle (Obstacle *o)
{
  double distFrom0
      = std::hypot (o->center ().x () - player0->coordinate ().x (),
                    o->center ().y () - player0->coordinate ().y ());
  double distFrom1
      = std::hypot (o->center ().x () - player1->coordinate ().x (),
                    o->center ().y () - player1->coordinate ().y ());
...
Player *
WarGame::generatePlayer (QString name, int width, int height)
{
  Player *player = new Player (name);

  // generise playera tkd se ne sece sa borderima
  QPointF playerPos = randomPoint (width - player->diameter (),
                                   height - 2 * player->diameter (), 0.2);
  player->setCoordinates (playerPos);

  return player;
}

// x-> width, y -> height, areaPercent -> procenat ekrana gde moze da se
// generise
QPointF
WarGame::randomPoint (int width, int height, double areaPercent)
{
...
void
WarGame::on_help_war_button_clicked ()
{
  ptrHelp->show ();
...
void
WarGame::clientReceivedMessage (QString msg)
{
  ...
  if (msg == "Player 2 has connected!" && m_fromCreate)
  ...
  else if (jsonObj["type"] == "setUpData" && !m_fromCreate)
    {
      ...
      QJsonObject coordinates0
          = player0Json.value ("m_coordinate").toObject ();
      ...
      QJsonObject coordinates1
          = player1Json.value ("m_coordinate").toObject ();
    ...
    {
      if ((m_fromCreate && currentPlayer == 1)
          || (!m_fromCreate && currentPlayer == 0))
        {
          ui->leFunctionInput->setDisabled (true);
          ui->fire_war_button->setDisabled (true);
        }
      ...
      ui->chat_textEdit->append (
          tr ("<font color=\"blue\"><i>") + jsonObj["player"].toString ()
          + " played y = " + jsonObj["data"].toString () + tr ("</i></font>"));
    }
  else if (jsonObj["type"] == "msg")
    {
      if (jsonObj["message"].toString () != "")
        ui->chat_textEdit->append (
            tr ("<font><b>") + jsonObj["name"].toString ()
            + tr (": </b></font>") + jsonObj["message"].toString ());
    }
}

void
WarGame::setClient (Client *newClient)
{
  m_client = newClient;
}
...
void
WarGame::on_quit_war_button_clicked ()
{
  ptrCheck->show ();
}
...
WarGame::eventFilter (QObject *obj, QEvent *event)
{
  if (obj == ui->fire_war_button && event->type () == QEvent::Enter)
  ...
  else if (obj == ui->help_war_button && event->type () == QEvent::Enter)
    {
      ...
      ui->help_war_button->setStyleSheet (
          backStyle + "border: 7px solid rgb(180, 72, 72);");
    }
