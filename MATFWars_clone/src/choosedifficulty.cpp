...
ChooseDifficulty::ChooseDifficulty (QWidget *parent)
    : QDialog (parent), ui (new Ui::ChooseDifficulty)
{
  ui->setupUi (this);
  ...
  connect (ptrGuessGame, &GuessGame::backGuessClicked, this,
           &ChooseDifficulty::show);
  connect (ptrGuessGame, &GuessGame::menuGuessFromResult, this,
           &ChooseDifficulty::on_back_difficulty_button_clicked);
}
...
void
ChooseDifficulty::on_go_difficulty_button_clicked ()
{
...
bool
ChooseDifficulty::eventFilter (QObject *obj, QEvent *event)
{
  if (obj == ui->go_difficulty_button && event->type () == QEvent::Enter)
    {
      ui->go_difficulty_button->setCursor (Qt::PointingHandCursor);
      ui->go_difficulty_button->setStyleSheet (
          goStyle + "border:4px solid rgb(180, 72, 72);");
    }
  else if (obj == ui->back_difficulty_button
           && event->type () == QEvent::Enter)
    {
      ui->back_difficulty_button->setCursor (Qt::PointingHandCursor);
      ui->back_difficulty_button->setStyleSheet (
          backStyle + "border:4px solid rgb(180, 72, 72);");
    }
