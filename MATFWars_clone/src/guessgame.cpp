...
GuessGame::GuessGame (QWidget *parent)
    : QDialog (parent), ui (new Ui::GuessGame), m_canvas (new Canvas (this))
{
  ui->setupUi (this);
  ...
  connect (ui->enter_guess_button, &QPushButton::clicked, this,
           &GuessGame::checkAnswerAndSetNewFunction);
  connect (ui->leFunctionInput, &QLineEdit::returnPressed, this,
           &GuessGame::checkAnswerAndSetNewFunction);
  connect (this, &GuessGame::newFunctionIsSet,
           dynamic_cast<Canvas *> (m_canvas), &Canvas::setFunction);

  connect (m_timer, SIGNAL (secPassed ()), this, SLOT (showTime ()));
  connect (m_timer, SIGNAL (timerExpired ()), this, SLOT (resultWindow ()));

  connect (ptrResult, &Result::menuResultClicked, this,
           &GuessGame::menuGuessFromResult);
...
void
GuessGame::resizeEvent (QResizeEvent *event)
{
...
void
GuessGame::setDifficulty (difficulty mode)
{
  m_diff = mode;
}
...
void
GuessGame::on_back_guess_button_clicked ()
{
  m_timer->stopCount ();
...
{
  ui->timer_label->setText (
      QString::number ((m_timer->getSec ()) >= 0 ? (m_timer->getSec ()) : 0));
}
...
GuessGame::checkAnswerAndSetNewFunction ()
{
  ...
  Function *currentFunction
      = new Function (m_functions[m_currentFunctionIndex]);
...
void
GuessGame::chooseFunctionIndex ()
{
  int size = m_functions.size ();

  if (m_usedFunctions.size () == size)
    m_usedFunctions.clear ();

  while (m_usedFunctions.count (m_currentFunctionIndex) > 0
...
GuessGame::drawCurrentFunction ()
{
  ...
  newFunction->translatePointsObserverView (m_canvas->width () / 2,
                                            m_canvas->height () / 2);

  QRectF sceneRect = QRectF (m_canvas->sceneRect ());

  FunctionNode *node = new FunctionNode (newFunction, sceneRect.width (),
                                         sceneRect.height ());
...
void
GuessGame::readFunctionsFromFile (std::string fileName)
{
...
difficulty
GuessGame::getDifficulty () const
{
  return m_diff;
}

void
GuessGame::resultWindow ()
{
  m_timer->stopCount ();
