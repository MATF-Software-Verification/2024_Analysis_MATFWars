...
void
Settings::on_func_color_button_clicked ()
{
...
void
Settings::on_obst_color_button_clicked ()
{
  QColor newColor = QColorDialog::getColor (Qt::white);
  if (newColor.isValid ())
    {
...
bool
Settings::eventFilter (QObject *obj, QEvent *event)
{
  if (obj == ui->func_color_button && event->type () == QEvent::Enter)
    {
      ui->func_color_button->setCursor (Qt::PointingHandCursor);
      ui->func_color_button->setStyleSheet (
          funcStyle + "border:4px solid rgb(180, 72, 72);");
    }
  ...
  else if (obj == ui->obst_color_button && event->type () == QEvent::Enter)
    {
      ...
      ui->obst_color_button->setStyleSheet (
          obstStyle + "border:4px solid rgb(180, 72, 72);");
    }
